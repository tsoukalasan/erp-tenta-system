...
import { 
  KATHETO_VT_BASE_PRICES,
  getPriceFromTable
} from '../../constants/pergolaPricingTables';

function KathetoVTConfig({ product, onSave }) {
  ...
  const calculatePrices = () => {
    ...
    if (config.width < 150 || config.width > 450) {
      newErrors.push('Το πλάτος πρέπει να είναι μεταξύ 150-450cm');
    }
    if (config.height < 150 || config.height > 350) {
      newErrors.push('Το ύψος πρέπει να είναι μεταξύ 150-350cm');
    }

    // Στρογγυλοποίηση: Πλάτος ανά 50cm, Ύψος ανά 25cm
    const widthOptions = [150, 200, 250, 300, 350, 400, 450];
    const heightOptions = [150, 175, 200, 225, 250, 275, 300, 325, 350];
    ...
    const basePrice = getPriceFromTable(KATHETO_VT_BASE_PRICES, roundedHeight, roundedWidth);
    if (basePrice === 0) {
...
export default KathetoVTConfig;
