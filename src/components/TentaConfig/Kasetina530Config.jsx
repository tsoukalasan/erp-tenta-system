...
function Kasetina530Config({ product, onSave }) {
  ...
  const roundUpToNext = (value, increment, min, max) => {
    if (value < min) return min;
    if (value > max) return max;
    return Math.ceil((value - min) / increment) * increment + min;
  };
  ...
  const calculatePrices = () => {
    ...
    const basePrice = getPriceFromTable(KASETINA_530_BASE_PRICES, roundedProjection, roundedWidth);
    if (basePrice === 0) {
    ...
    // Επιβάρυνση χρώματος +160€ αν custom
