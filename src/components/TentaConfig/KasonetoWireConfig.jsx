...
import { 
  KASONETO_WIRE_BASE_PRICES,
  getPriceFromTable
} from '../../constants/pergolaPricingTables';

function KasonetoWireConfig({ product, onSave }) {
  ...
  const calculatePrices = () => {
    ...
    if (config.width < 175 || config.width > 400) {
      newErrors.push('Το πλάτος πρέπει να είναι μεταξύ 175-400cm');
    }
    if (config.height < 175 || config.height > 300) {
      newErrors.push('Το ύψος πρέπει να είναι μεταξύ 175-300cm');
    }

    // Στρογγυλοποίηση ανά 25cm
    const widthOptions = [175, 200, 225, 250, 275, 300, 325, 350, 375, 400];
    const heightOptions = [175, 200, 225, 250, 275, 300];
    ...
    const basePrice = getPriceFromTable(KASONETO_WIRE_BASE_PRICES, roundedHeight, roundedWidth);
    if (basePrice === 0) {
...
export default KasonetoWireConfig;
