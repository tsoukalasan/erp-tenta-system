...
const PergolaPro150Config = ({ product, onSave, onNavigate }) => {
  ...
  const calculatePrices = () => {
    ...
    basePrice = getPriceFromTable(PRO150_BASE_PRICES, config.projection, config.width);
    console.log('🔢 Υπολογισμός PRO150:', { 
    ...
    const roundedWidth = roundUpToNextAvailableWidth(PRO150_BASE_PRICES, config.projection, config.width);
    ...
    if (config.parapet === 'lux') {
      ...
      // Επιβάρυνση για κολόνες (μόνο αν ΔΕΝ είναι 130x100)
      if (config.columnType === '150x100') {
        columnSurcharge = columnCount * 50; // 50€ ανά κολόνα
      } else if (config.columnType === '150x150') {
        columnSurcharge = columnCount * 100; // 100€ ανά κολόνα
      }
      ...
      // Κιλοδοκός: Κολόνες πάντα 80×80, χωρίς επιλογή τύπου
  ...
  const handleSave = () => {
    ...
    <div className="max-w-6xl mx-auto p-6 bg-white rounded-lg shadow-lg">
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 mt-1">
              Στρογγυλοποίηση: {roundUpToNextAvailableWidth(PRO150_BASE_PRICES, config.projection, config.width)}cm
            </p>
          </div>
          ...
          <div>
            ...
            />
            <p className="text-xs text-gray-500 mt-1">
              Στρογγυλοποίηση: {roundUpToNextAvailableProjection(PRO150_BASE_PRICES, config.projection)}cm
            </p>
          </div>
      ...
      </div>      <div className="mt-8 p-6 bg-blue-50 border border-blue-200 rounded-lg">
...
export default PergolaPro150Config;
